...
namespace IQF.Framework
{
	...
	public struct Date : IComparable, IFormattable, ISerializable, IComparable<Date>, IEquatable<Date>
	{
		...
		/// <summary>
		/// 除了ToDateTime，其实Date类型也可以隐式的转换为DateTime类型
		/// </summary>
		/// <returns></returns>
		...
		/// <summary>
		/// 解析形如20150702这样格式的日期，失败抛异常
		/// </summary>
		/// <param name="s"></param>
		/// <returns></returns>
		public static Date ParseIntString(string s)
		{
			var date = ParseExact(s, "yyyyMMdd");
			return date;
		}

		public static Date ParseExact(string s, string format)
		{
			var date = ParseExact(s, format, System.Globalization.CultureInfo.InvariantCulture);
			return date;
		}
